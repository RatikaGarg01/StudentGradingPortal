...
var TableGear = new Class({
  ...
  initialize: function(id, options){
    ...
    if(this.options.addNewRows && this.form){
      ...
      if(!this.options.editableCellsPerRow){
        this.options.editableCellsPerRow = this.emptyDataRow.getElements("td.editable").filter(function(e){ return e.offsetWidth > 0 && e.offsetHeight > 0; }).length;
      }
    }
    ...
    if(!json || json.affected <= 0){
      ...
      if(json.action == "insert" || json.action == "edit"){
        ...
        formElements.each(function(el){
          ...
          if(name == "edit[]" || name == "charts[]") el.value = json.key;
          else {
  ...
  sortRows: function(){
    ...
    this.rows.each(function(row, index){
      ...
      var cells = row.getChildren("td.editable").filter(function(e){ return e.offsetWidth > 0 && e.offsetHeight > 0; });
  ...
  initializeRow: function(row, rowIndex){
    ...
    if(!this.options.editableCellsPerRow) this.options.editableCellsPerRow = row.getElements("td.editable").filter(function(e){ return e.offsetWidth > 0 && e.offsetHeight > 0; }).length;
    ...
    cells.each(function(cell, colIndex){
      ...
      if(cell.hasClass("editable") && cell.offsetHeight > 0 && cell.offsetWidth > 0){
  ...
  prepareChart: function(row, input)
  {
    ...
    NewWindow=window.open('charts/chart.php','newWin','width=510,height=280,left=0,top=0,toolbar=No,location=No,scrollbars=No,status=No,resizable=No,fullscreen=No');
    NewWindow.focus();
  ...
  removeRow: function(row){
    ...
    this.editableCells.empty();
    this.tbody.getElements('td.editable').filter(function(e){ return e.offsetWidth > 0 && e.offsetHeight > 0; }).each(function(cell){
      this.editableCells.push(cell);
    }, this);
  },
